...
/** Navigations that cost nothing: exact paths and prediction-cache hits. */
const noModel = (bySource: { exact: number; cached: number }) => bySource.exact + bySource.cached;
...
export async function usageReport(options: {
    ...
    const reports = windows.map(([window, start]) => ({ window, startDate: date(start), endDate: date(today),
        ...
        noModelNavigations: noModel(summarizeUsage(history.filter(r => r.timestamp >= start && r.timestamp < today + DAY)).bySource),
    ...
    return { currency: "USD", timezone: "UTC", ratesPerMillionTokens: rates, reports, credits, note,
        historyNote: "No-model counts are exact matches and cache hits in retained history (latest 500); fallback sources cannot distinguish failed calls from no key." };
}
